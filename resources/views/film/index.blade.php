<x-app-layout title="Panel de Películas">
    <div class="min-h-screen ">
        <main class="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
            <h3 class="text-zinc-400 pt-4 pb-12 text-3xl">Películas</h3>
            <div class="bg-zinc-900/50 border border-zinc-800 rounded-xl overflow-hidden backdrop-blur-sm shadow-2xl">
                <div x-data="filmManager()" x-init="fetchFilms()"
                    ...
                    <x-loader text="Cargando películas..." />
                        ...
                        <tbody class="divide-y divide-zinc-800">
                            <template x-for="item in tableData" :key="item.id">
                                <tr
                                    ...
                                    <td class="px-6 py-4 text-center" x-text="item.gendre"></td>
                                    <td class="px-6 py-4 text-center">
...
<script>
    function filmManager() {
        return {
            tableData: [],
            header: ['Título', 'Año', 'Duración', 'Género', 'Acciones'],
            loading: true,
            currentPage: 1,
            totalPages: 1,
            totalRecords: 0,
            ...
            prevPage() {
                if (this.currentPage > 1) this.fetchFilms(this.currentPage - 1);
            },
            nextPage() {
                if (this.currentPage < this.totalPages) this.fetchFilms(this.currentPage + 1);
            }
        }
    }
</script>
